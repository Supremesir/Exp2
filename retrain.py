...
def mian():
    ...
    #加载MobileNet模型
    init_model = tf.keras.applications.mobilenet.MobileNet(input_shape=(224,224,3), include_top=False,
                                                           weights='imagenet',
                                                           poolings='avg')
    #在网络顶层进行fine-tune操作
    ...
    model = tf.keras.Model(inputs=init_model.input, foutputs=output)
